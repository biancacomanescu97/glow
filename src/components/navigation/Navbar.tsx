...
export default function Navbar() {
  return (
    <Disclosure as="header" className="absolute z-10 inset-x-0 top-4"> 
      {({ open }) => (
        <>
          <div className="flex">
            {/* Mobile menu button */}
          ...
          <Disclosure.Panel className="p-4 min-h-screen text-royal-600 text-base font-medium bg-floralwhite-100">
            {navigationItems.map((item) => (
              <Link to={item.href} className="block p-3 hover:text-royal-300">
                {item.name}
              </Link>
            ))}
          </Disclosure.Panel>
